...
router.post("/upload", imageMiddleware, (req, res) => {
	try {
		if (!req.file) {
			return res.status(400).json(createResponse(400, "업로드된 파일이 없습니다."));
		}
		...
		res.status(200).json(createResponse(200, "파일 업로드 성공!", { 
			originalUrl, 
			thumbnailUrl 
		}));
	} catch (error) {
		console.error("파일 업로드 오류:", error);
		res.status(500).json(createResponse(500, "파일 업로드 실패!", { error: error.message }));
	}
});
...
router.delete("/delete/:id", async (req, res) => {
	...
	if (!fileFolder || !fileName) {
		return res.status(400).json(createResponse(400, "파일 폴더 또는 파일 이름이 제공되지 않았습니다."));
	}
	
	const filePath = path.join(uploadDir, fileFolder, fileName);
	const thumbnailPath = path.join(uploadDir, fileFolder, `thumb_${fileName.split('.')[0]}.jpeg`);
	
	try {
		const fileDeleted = await deleteFileIfExists(filePath);
		if (!fileDeleted) {
			return res.status(404).json(createResponse(404, "파일을 찾을 수 없습니다."));
		}
		...
		res.status(500).json(createResponse(500, ERROR_MESSAGE, { error: err.message }));
	}
});
